...
export default function Admin() {
  ...
  const handleInstructorChange = useCallback(
    (day, index, newValue) => {
      handleClassChange(day, index, "Instructor", newValue?.value || "");
    },
    [handleClassChange]
  );
  ...
  // Change fetchIDs to use fetchAllIDs for consistency
  ...
  const getEmptySchedule = useCallback(() => {
    return {
      Monday: [],
      Tuesday: [],
      Wednesday: [],
      ...
      try {
        ...
        if (error.response) {
          ...
          if (error.response.status === 404) {
            ...
            toast.error(
              error.response.data.message || "Error fetching instructors"
            );
          }
  ...
  useEffect(() => {
    if (
      ...
      if (formData.selectedID) {
        ...
        setFormData((prev) => ({
          ...prev,
          ID: `${formData.university}-${formData.program}-${formData.semester}-${formData.section}`,
        }));
      }
    } else {
      setFormData((prev) => ({
  ...
  }, [formData.selectedID, fetchScheduleByID]);
    ...
    if (formData.ID) {
      try {
        const response = await axios.delete(
          `/api/schedules/delete/${formData.ID}`
        );
        ...
        toast.error(`Error: ${error.message || "Failed to delete schedule"}`);
      }
    }
  }, [formData.ID, fetchAllIDs, getEmptySchedule]);
